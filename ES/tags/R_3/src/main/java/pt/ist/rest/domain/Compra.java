...
public class Compra extends
     ...
     */
    public void adicionaPrato(Prato p, int quantidade) {
        if (hasPrato(p)) {
            adicionaPratoEmItemExistente(p, quantidade);
        } else {
     ...
     * @param quantidade the quantidade
     */
    private void adicionaPratoEmItemExistente(Prato p, int quantidade) {
        for (Item i : getListaItens()) {
            if (i.get_Prato().getId() == p.getId()) {
