...
import pt.ist.rest.exceptions.InvalidNifException;
...
public class Cliente extends
     ...
     * @param nif the nif
     */
    public void registaNIF(int nif) throws InvalidNifException {
        if (isValidNif(nif)) {
            set_nif(nif);
        } else {
            throw new InvalidNifException("O nif do cliente nao e valido!",this.getUsername(), nif); 
        }
    }
    ...
    public void fechaTabuleiro() {
        Compra tabuleiro = getTabuleiro();
      //  tabuleiro.passaFactura();
        tabuleiro.setRegisto(true);
        getPortalRestaurante().set_idLastTabuleiroFechado(tabuleiro.getIdCompra());
    }
}
