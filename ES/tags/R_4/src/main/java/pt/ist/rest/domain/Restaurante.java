...
public class Restaurante extends
    ...
    public void adicionaPrato(String nomePrato,String tipo,Double precoPrato,Double caloriasPrato,Integer idPrato) throws RestauranteMaxPratosException, RestauranteJaContemPratoException, MaxPrecoPratoException{
            if (getPratoSet().size() < getPortalRestaurante().get_maxPratosPorRestaurante()) {
                if (contemPrato(nomePrato)) {
                    ...
                    if(precoPrato > getPortalRestaurante().get_maxPrecoPrato()){
                        throw new MaxPrecoPratoException("O prato tem um preco superior ao maximo permitido",getPortalRestaurante().get_maxPrecoPrato(),nomePrato,precoPrato);
                    }else{
                        Prato p = new Prato(nomePrato,tipo, precoPrato, caloriasPrato,idPrato);
                        addPrato(p);
                    }
                }
            } else {
